...
## OTA update mechanism (brought into v0.1)

To avoid rebuild-sideload-reinstall friction on every change, the app checks a small public manifest (a JSON file with the latest web-bundle version + download URL) and swaps its `www` folder if a newer bundle is available. This only covers changes to the web bundle (React/UI/logic) — any change to native code or plugins still requires a full APK rebuild and reinstall. Sign the APK consistently with the same key across builds so Android treats new installs as upgrades rather than "unknown app" installs, minimizing Play Protect friction to a one-time approval.

Once Drive integration lands (v0.2), the update manifest can move to the user's own Drive folder instead of a separate static host — no third-party OTA service (Capgo/Appflow) needed.
